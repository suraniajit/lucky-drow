        ...
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-sm-1">
                    ...
                    <button type="button" id="add_booking_button" class="btn btn-info btn-lg" data-toggle="modal"
                        data-target="#add_booking_modal">+</button>
                    @endcan
                </div>
            </div>
            <div class="card-body">
...
<template id="tiket-row-template">
    <div class="form-row booking_symbole_div symbole_row ">
        ...
        <div class="form-group col-md-3">
            ...
            <input type="number" min=0 name="symbole_booking_count[]" class="form-control symbole_booking_count "
                onchange="changeSymboleCount(this);" value="0">
        </div>
        <div class="form-group col-md-3">
...
@can('admin.booking.create')
<div class="modal fade" id="add_booking_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLongTitle">{!! __('booking::booking/labels.add-new-booking') !!}
                </h5>
                    ...
                    <div class="form-row">
                        <div class="form-group col-md-12">
                            ...
                            <input type="text" name="mobile" class="form-control "
                                placeholder="{!! __('booking::booking/labels.booking_mobile') !!}">
                        </div>
                    </div>
                    <div id="tiket_detail">
                    </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">{!!
                    __('core::core/labels.core-form-button-close') !!}</button>
                <button type="button" class="btn btn-primary" id="book_submit">{!!
                    __('core::core/labels.core-form-button-save') !!}</button>
            </div>
        </div>
    </div>
</div>
            ...
            if (data.status == 'Success') {
                if (data.data.length > 0) {
                    ...
                    for (i = 0; i < data.data.length; i++) {
                        ...
                        clone.querySelector(".id").innerHTML = ((data.proparty.current_page - 1) * data
                            .proparty.per_page) + i + 1;
                        clone.querySelector(".booking_id").innerHTML = data.data[i].booking_id;
                        clone.querySelector(".balance_transaction_id").innerHTML = data.data[i]
                        ...
                        @can('admin.booking.view')
                        var delete_str =
                            '<button type="button" onclick="deleteShow(this)" class="btn btn-danger btn-sm"  data-id="' +
                            data.data[i].id + '">' +
                            '<i class="fa fa-trash" aria-hidden="true"></i>' +
                            '</button>';
                        action_str = action_str + '' + delete_str;
                        @endcan
                        @can('admin.booking.edit')
                        edit_button = '';
                        var edit_button = '<button type="button" onClick="getEditBooking(this)" data-id="' +
                            data.data[i].id + '" class="btn btn-info btn-sm" >' +
                            '<i class="fa fa-pencil" aria-hidden="true"></i>' +
                            '</button>';
                        action_str = action_str + '' + edit_button;
                        @endcan
                        @can('admin.booking.delete')
                        var delete_str =
                            '<button type="button" onclick="deleteBooking(this)" class="btn btn-danger btn-sm"  data-id="' +
                            data.data[i].id + '">' +
                            '<i class="fa fa-trash" aria-hidden="true"></i>' +
                            '</button>';
                        action_str = action_str + '' + delete_str;
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.status == 'Success') {
                var option_string = '';
                for (i = 0; i < data.data.length; i++) {
                    option_string = option_string + '<option value="' + data.data[i].id + '">' +
                        data.data[i].time + '</option>';
                }
            ...
            if (data.status == 'Success') {
                ...
                if (data.data.length > 0) {
                    ...
                    for (i = 0; i < data.data.length; i++) {
                        ...
                        clone.querySelector(".symbole_id_hidden_val").setAttribute('value', data.data[i]
                        .id);
...
@endCan
@can('admin.booking.edit')                       
function getEditBooking(e) {
    id = $(e).attr('data-id');
    url = "{{ url('') }}" + '/api/booking/edit/' + id;
    var token = window.localStorage.getItem('token');
    $.ajax({
        type: 'get',
        url: url,
